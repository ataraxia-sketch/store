...
form.onsubmit=async(e)=>{e.preventDefault(); const text=input.value.trim(); if(!text)return; await ensureChat(); await addDoc(msgRef,{text,sender:'customer',createdAt:serverTimestamp()}); await setDoc(chatRef,{customerName:customerName||'Tamu',lastMessage:text,updatedAt:serverTimestamp(),status:'open'},{merge:true}); input.value=''};
